...
public class TemplateBuilder {
    ...
    static {
        try {
            ...
            //获取对应的配置信息
            PROJPACKAGE = props.getProperty("ProjPackage");
            PACKAGE_POJO = PROJPACKAGE+".pojo";
            PACKAGE_MAPPER = PROJPACKAGE+".dao";//props.getProperty("mapperPackage");
            PACKAGE_SERVICE_INTERFACE =PROJPACKAGE+".service";// props.getProperty("serviceInterfacePackage");
            PACKAGE_SERVICE_INTERFACE_IMPL = PROJPACKAGE+".service.impl";//props.getProperty("serviceInterfaceImplPackage");
            PACKAGE_CONTROLLER = PROJPACKAGE+".controller";//props.getProperty("controllerPackage");
            PACKAGE_FEIGN= PROJPACKAGE+".feign";//props.getProperty("feignPackage");
            PACKAGE_ENTITY=PROJPACKAGE+".entity";
        ...
        try {
            ...
            if(databaseType.equals("MySQL")){
                ...
                while (tableResultSet.next()){
                    ...
                    modelMap.put("Date",DateUtil.date2String(new Date(),"yyyy-MM-dd HH:mm:ss"));
                    modelMap.put("TableName",tableName);
                    modelMap.put("models",models);
                    modelMap.put("typeSet",typeSet);
                    modelMap.put("projPackage",PROJPACKAGE);
                    //主键操作
                    if (key!=null&&key!=""){
...
//                    swaggerPathList.addAll(swaggerMethodInit(Table,table,StringUtils.firstLower(keyType),format));
        ...
        modelMap.put("serviceName",UpFirstChar(SERVICENAME));
        ApplicationBuilder.builder(modelMap);
